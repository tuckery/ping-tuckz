...
def _build_result_page_metadata(results_dir=OUTPUT_DIR, current_html_path=None):
    pages = []
...
def output_chunk(title, all_indices):
    ...
    if has_abnormalities:
        ...
        # Use chunk_metadata_list's length, not event_html_list's: after a reload of an
        # existing HTML file, _extract_event_html collapses all prior chunks into a single
        # combined blob (event_html_list has 1 entry), while _recover_chunk_metadata still
        # parses out every individual chunk (chunk_metadata_list stays accurate). Basing the
        # id on event_html_list's length would reissue already-used ids and desync them from
        # the nav widget order, breaking chunk-nav click navigation.
