...
def open_day_files(dt):
    ...
    appending = os.path.exists(txt_path) and os.path.getsize(txt_path) > 0
    if appending and total_pings > 0:
        print(f"Appending to existing day file ({total_pings} pings already recorded today)")
...
print(f"Today's files: {os.path.basename(TXT_LOG_FILE)} / {os.path.basename(HTML_LOG_FILE)}")
...
if platform.system() == 'Windows':
    ping_cmd = ['ping.exe', '-l', '64', '-t', ping_target]
else:
    # macOS/Linux: -s for packet size (56 + 8 header = 64), pings continuously by default
    ping_cmd = ['ping', '-s', '56', ping_target]
...
print(f"Real-time monitoring started for {ping_target} (all abnormalities grouped by 20s gaps). Press Ctrl+C to stop.")
print(f"Live Stream: <50ms default [NORMAL]; 50-100ms {ORANGE}orange{RESET} [MEDIUM]; >100ms {RED}red{RESET} [HIGH].\n")
...
try:
    for line in proc.stdout:
        ...
        if now.date() != current_day_date:
            ...
            # Reset chunk-tracking and session stats for the new day
            _reset_chunk_state()
            _reset_session_stats()

            # Open new day's files
            open_day_files(now)
            print(f"\n--- Midnight crossed. Now writing to: {os.path.basename(TXT_LOG_FILE)} / {os.path.basename(HTML_LOG_FILE)} ---\n")
        ...
        if is_timeout(line):
            ...
            stream_line = f"{RED}[TIMEOUT] {formatted_line}{RESET}"
            print(stream_line)
            ...
            continue  # Skip latency stats

        # Parse latency (non-timeout)
        latency = parse_latency(line)
        if latency is None:
            continue
        ...
        if latency < 50:
            ...
            colour = RED
            status = 'HIGH'
        prefix = f"[{status}: {latency}ms] "
        stream_line = f"{colour}{prefix + formatted_line}{RESET if colour else ''}"
        print(stream_line)
        ...
        if latency < SPIKE_THRESHOLD:  # Normal ping
            ...
            # Add to all abnormalities list for HTML display
            all_abnormalities_list.append((current_idx, latency, formatted_line))
            # Add to current chunk abnormalities
